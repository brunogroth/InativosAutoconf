...
@section('content')
    ...
    <div class="site-filter">
        <form class="row gx-3 gy-2 align-items-center" method="get" action="{{ route('filter') }}">
            ...
            <div class="col-auto">
                <a href="{{ route('list') }}" class="btn btn-danger" type="submit"> <i class="fa fa-trash"
        ...
        <div class="row justify-content-end">
            <div class="col-4">
                <nav aria-label="Page navigation example">
                    <ul class="pagination">
                        @foreach ($sites->links as $pagination)
                            <li class="page-item">
                                <a href="{{ $pagination->url }}" class="{{ $pagination->active ? 'active' : '' }}">
                                    {{str_contains($pagination->label,'&laquo;') ? 'Anterior' : $pagination->label }}
                                </a>
                            </li>
                        @endforeach
